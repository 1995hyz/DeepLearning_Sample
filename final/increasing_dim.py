...
class Model(tf.Module):
    def __init__(self, dimensionality, load_flag=0):
        ...
        if not load_flag:#This allows us to not overwrite our saved weights
...
if __name__ == "__main__":
    data = Data()
    dim_list = [100, 200, 400, 600, 800, 1000, 1200, 1400 ]
    big_accuracy_track = np.full((NUM_BATCHES*7, 1), np.nan)
    big_accuracy_track_test = np.full((NUM_BATCHES*7, 1), np.nan)
    k = 0 #Iterator for big_trackers
    for dim in dim_list:
        learn_rate = dim**-1 #Lets see if this works...
        load = 0
        if dim > 100:
            load = 1
        ...
        for i in bar:
            ...
            big_accuracy_track[k] = accuracy
            big_accuracy_track_test[k] = accuracy_test
            k = k + 1 #increment k
    ...
    plt.plot(big_accuracy_track)
    plt.plot(big_accuracy_track_test)
    plt.title("Big Accuracy Tracker")
    plt.show()
    f.close()
