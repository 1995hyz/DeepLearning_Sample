...
class Data(object):
    ...
    def __init__(self):
        ...
        test_images, test_lables = loadlocal_mnist(
            test_images_path, test_lables_path
        )
        self.training = (
            train_images[self.choices[:50000]] / 255
        ).reshape(
            -1, 28, 28, 1
        )  # Normalize Value
        self.validation = (
        ...
        self.test = (test_images / 255).reshape(
            -1, 28, 28, 1
        )
        self.labels_train = np.eye(NUM_CLASSES)[
            train_labels[self.choices[:50000]]
        ]
        self.labels_validate = np.eye(NUM_CLASSES)[
            train_labels[self.choices[50000:]]
        ]
